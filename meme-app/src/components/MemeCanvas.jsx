...
const MemeCanvas = forwardRef((props, ref) => {
  ...
  if (!image) {
    return (
      <div className={styles.memeCanvas}>
  ...
  return (
    <div className={styles.memeCanvas}>
      <div className={styles.memeDisplay} ref={overlayRef}>
        <img
          src={image.src}
          alt="Meme"
        ...
        >
          {textBlocks.map((block) => {
            ...
            return (
              <div
                key={block.id}
                className={`${styles.memeTextWrapper} ${isSelected ? styles.selected : ''} ${draggingId === block.id ? styles.dragging : ''} ${resizingId === block.id ? styles.resizing : ''}`}
                style={{
                  left: `${block.x * 100}%`,
                  top: `${block.y * 100}%`,
              ...
              >
                {isSelected ? (
                  <textarea
                    ...
                    className={`${styles.memeTextBlock} ${styles.memeTextEdit}`}
                    dir="ltr"
                    value={block.content}
                  ...
                  <div
                    className={styles.memeTextBlock}
                    onMouseDown={(e) => {
                      e.stopPropagation()
                      onSelectText(block.id)
      ...
      <canvas ref={ref} className={styles.exportCanvas} aria-hidden="true" />
    </div>
  )
})
